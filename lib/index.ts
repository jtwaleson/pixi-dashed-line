...
export class DashLine {
    ...
    private clipRect?: { x: number; y: number; width: number; height: number }
    ...
    private adaptiveBezierSubdivision(
        ...
        const points: Point[] = [new Point(x0, y0)]
    ...
    private clipLine(x0: number, y0: number, x1: number, y1: number): [number, number, number, number] | null {
        if (!this.clipRect) return [x0, y0, x1, y1]
