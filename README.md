...
* Dashed support for lineTo, circle, ellipse, poly, rect, and roundRect
...
DashLine.DashLineOptions = {
    ...
    dash?=[10, 5] - an array holding one or more [dash, gap] entries, eg, [10, 5, 20, 10, ...])
    width?=1 - width of the dashed line
    color?=0xffffff - color of the dashed line
    alpha?=1 - alpha of the dashed line
    scale?=1 - scale of the dashed line
    cap? - add a line cap style to dashed lines (only works for useTexture: false)
    join? - add a line join style to the dashed lines (only works for useTexture: false)
    alignment? - change alignment of lines drawn (0.5 = middle, 1 = outer, 0 = inner)
}
```
...
#### circle(x: number, y: number, radius: number, points=80, matrix?: Matrix)
...
#### ellipse(x: number, y: number, radiusX: number, radiusY: number, points=80, matrix?: Matrix)
...
#### rect(x: number, y: number, width: number, height: number, matrix?: Matrix)
...
#### roundRect(x: number, y: number, width: number, height: number, cornerRadius?: number, matrix?: Matrix)
draws a dashed rounded rectangle; matrix is applied before the draw (this adds a shape-specific transform to pixi's DisplayObject transforms)

#### poly(Point[] | number[], matrix?: Matrix)
...
#### setStrokeStyle()
changes line style to the proper dashed line style -- this is useful if the graphics element's lineStyle was changed. Note: This is now an async method when using textures.
